...
class Item:
	...
	def __init__(self, owner=None, typeo=None, title=None, uniqid=None, artist=None, genre=None, year=None, isOld=None):
		...
		db.commit()
		cursor.execute("update items set item_id=? where owner=? and title=?",(self.owner,self.title,self.item_id))
		db.commit()
		db.close()
	...
	def borrowedreq(self, user):
		...
		# else:
		return "You cannot make a borrow request for this item."
		db.close()
	...
	def comment(self, user, comment_text):
		...
		db = sqlite3.connect("database.db")
		cursor = db.cursor()
		stri="You cannot make a comment for this item."
		notifs=[]
		if self.can_look_at_it(user, "comment"):
			...
			# For watch method:
			cursor.execute("select user_email from watch where (item_id=? and type=?)", (self.item_id, "detail"))
			for user_email in cursor:
				notification_text = "Notification to " + user_email[0] + " : " + user + "commented to " + self.title;
				...
				cursor.execute(
					...
					(user_email[0], notification_text, notification_type))
				db.commit()
		db.close()
		print(notifs)
		return (stri,notifs)
	...
	def borrowedby(self, user, returndate):
		...
		data = cursor.fetchall()
		#req atmıs mı
		stri="You cannot because there is no request from that user."
		notifs=[]
		if data:
			print("fetched")
			...
			for x in cursor:
				if x[0] == 0:
					stri= "It is already borrowed by "+x[1]+"."
					return(stri,notifs)
			...
			cursor.execute("select user_email from watch where (item_id=? and type=?)", (self.item_id, "borrow"))
			for user_email in cursor:
				notification_text = "Notification to " + user_email[0] + " : " + user + " borrowed " + self.title;
				notifs.append(notification_text)
				notification_type = 1;  # comment
				cursor.execute(
					"insert into notifications(user_email,notification_text,notification_type) values(?,?,?)",
					(user_email[0], notification_text, notification_type))
				db.commit()
	...
	def returned(self,location=None):
		...
		cursor = db.cursor()
		taker=""
		cursor.execute("select user_email from itemborrows where item_id=? and is_returned=?", (int(self.item_id),0))
		for x in cursor:
			taker=x[0]
		...
		cursor.execute("delete from itemrequests where item_id=? and user_email=?",(self.item_id,taker))
		db.commit()
	...
	def listcomments(self):
		...
		comment_user_list = []
		for x in cursor:
			comment_user_list.append(x[0] + " : " + x[1])
		db.close()
	...
	def watch(self,user, watch_method):
		...
		if self.owner==user:
			...
			db.commit()
			return "You started to watch this item."
			db.close()
	...
	def can_look_at_it(self, user, comment_borrow):
		...
		cursor.execute("select *from friendships where\
			(User1=? and User2=? and isClose=?)\
			 or \
			(User2=? and User1=? and isClose=?) or (User1=? and User2=? and isClose=?) or (User2=? and User1=? and isClose=?)", (self.owner, user, 1, self.owner, user, 2,self.owner,user,3,self.owner,user,3))
		count = 0
		for x in cursor:
			count += 1
		if count != 0:
		...
		if comment_borrow == "comment":
			attribute = self.comment_at
		if comment_borrow == "borrow":
			attribute = self.borrow_at
		elif comment_borrow == "view":
			attribute = self.view_at
		elif comment_borrow == "search":
			attribute = self.search_at
		elif comment_borrow == "detail":
			attribute = self.detail_at
	...
	def view(self,user):
		msg = "You do not have the right of view the item."
		if self.can_look_at_it(user,"view"):
			msg = "Title: " + str(self.title) + ", Artist: " + str(self.artist) + ", Year: " + str(self.year)
		return msg

	def detail(self,user):
		msg = "You do not have the right of detail information of the item."
		location = ""
		if user == self.owner:
			location = self.location
		if self.can_look_at_it(user,"detail"):
			msg = "Title: " + str(self.title) + ", Artist: " + str(self.artist) + ", Year: " + str(self.year) + ", Owner: " + self.owner + ", Location: " + location
		return msg
	def delete(self):
		db = sqlite3.connect("database.db")
		cursor = db.cursor()
