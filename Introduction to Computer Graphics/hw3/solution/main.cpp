...
GLuint idMVPMatrix;
int widthTexture, heightTexture, leftrightamount, lightY=100, lightX=0, lightZ=0;
...
glm::vec3 cam_gaze = glm::vec3(0.0, 0.0, 1.0);
...
static void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
    if (key == GLFW_KEY_Q)
		...
		cam_pos = glm::vec3(widthTexture/2,widthTexture/10,-1*widthTexture/4);

		cam_gaze = glm::vec3(0.0, 0.0, 1.0);
...
cam_pos=glm::vec3(widthTexture/2,widthTexture/10,-1*widthTexture/4);
lightX=widthTexture/2;
lightZ=heightTexture/2;
